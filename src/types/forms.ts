...
export const SignUpSchema: ZodType<FormData> = z
  .object({
    ...
    name: z.string().trim().min(1, "Full name is required.").min(3, "Full name must be at least 3 characters."),
    confirmEmail: z.string().trim().min(1, "Confirm Email is required.").email(),
  })
